class CellPhoneNumberInput < SimpleForm::Inputs::Base
  def input
    ActiveSupport::SafeBuffer.new.tap do |out|
        out << @builder.input(:cell_area_code, wrapper: :phone_input,
                      required: false, placeholder: '000', wrapper_html: { class: 'icon-phone grid-3-12' })
        out << "<span class='grid-1-24 type-sep no-break'>-</span>".html_safe
        out << @builder.input(:cell_prefix, wrapper: :phone_input, required: false,
                      placeholder: '000', wrapper_html: { class: 'grid-2-12' })
        out << "<span class='grid-1-24 type-sep no-break'>-</span>".html_safe
        out << @builder.input(:cell_line, wrapper: :phone_input, required: false,
